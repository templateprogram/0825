...
const remoteRouteNamePrefix="admin-fakeImage";
...
const dataUpdate = () => {
    ...
    if(props.theSingleData?.id)
    {
        ...
        })).put(route(`${remoteRouteNamePrefix}.update`, props?.theSingleData?.id), { preserveState: true, onSuccess: () => showDialogModal.edit = false });
    }
    else
    {
        dataStore();
    ...
    <AdminLayout>
        ...
        <main>
            ...
            <section v-for="_singleData in props.theData.data" v-if="props.theData.data[0]" :key="_singleData.id"
                ...
                <!-- 圖片 -->
                <section v-if="false" class="w-full md:max-w-[100px]">
                    ...
                    <template #content>
                        <form @submit.prevent="props.theSingleData.id ? dataUpdate() : dataStore()" class="space-y-6">
                            ...
                            <div v-for="(_ui,_columnName) in _formInputs" v-bind:key="_columnName">
                                ...
                                ></custom>
                                <!-- :imgAttributes="props?.theSingleData?.img_attributes" is for existsing attributes -->
                                <!-- @giveImageAttributes="giveImageAttributes" is for preview file -->
                                <!-- @giveImageAttributes="giveImgDataAttributes" is for existing file -->
